...
namespace live {
    ...
    public:
        ...
        [[nodiscard]] const std::list<std::pair<INodePtr, INodePtr>> &GetList() const {
            return move_list_;
        }
        void Append(INodePtr src, INodePtr dst) {
            move_list_.emplace_back(src, dst);
        }
        bool Contain(INodePtr src, INodePtr dst) const;
        void Delete(INodePtr src, INodePtr dst);
        void Prepend(INodePtr src, INodePtr dst) {
            move_list_.emplace_front(src, dst);
        }
        MoveList *Union(MoveList *list) const;     // 求并集
        MoveList *Intersect(MoveList *list) const; // 求交集

        // ----------------------------------------- 补充 -----------------------------------------

        // 返回新的
        [[nodiscard]] bool isEmpty() const;
        [[nodiscard]] MoveList Unioned(const MoveList &list) const;     // 求并集
        [[nodiscard]] MoveList Intersected(const MoveList &list) const; // 求交集

        // 返回引用
        MoveList &Union(const MoveList &list);     // 求并集
        MoveList &Intersect(const MoveList &list); // 求交集
    ...
    public:
        ...
        void Liveness();
        LiveGraph GetLiveGraph() {
            return live_graph_;
        }
        tab::Table<temp::Temp, INode> *GetTempNodeMap() {
