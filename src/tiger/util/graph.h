...
namespace graph {
    ...
    private:
        ...
        T *info_;
        Node<T>()
            : my_graph_(nullptr), my_key_(0), succs_(nullptr), preds_(nullptr), info_(nullptr) {
        }
    };
    ...
    public:
        ...
        [[nodiscard]] NodeList<T> Unioned(const NodeList<T> &list) const;     // 求并集
        [[nodiscard]] NodeList<T> Intersected(const NodeList<T> &list) const; // 求交集
        [[nodiscard]] NodeList<T> Diffed(const NodeList<T> &list) const;      // 求补集

        // 返回引用
        NodeList<T> &Union(const NodeList<T> &list);     // 求并集
        NodeList<T> &Intersect(const NodeList<T> &list); // 求交集
        NodeList<T> &Diff(const NodeList<T> &list);    // 求补集
    ...
    template <typename T>
    bool NodeList<T>::Contain(Node<T> *n) const {
    ...
    template <typename T>
    NodeList<T> *NodeList<T>::Diff(NodeList<T> *nl) {
        NodeList<T> *res = new NodeList<T>();
        for (auto node : node_list_) {
            if (!nl->Contain(node))
    ...
    template <typename T>
    NodeList<T> NodeList<T>::Unioned(const NodeList<T> &list) const {
        //        std::list<Node<T> *> new_node_list;
        //        for (auto node : node_list_) {
        //            new_node_list.push_back(node);
        //        }
        //        for (auto node : list.node_list_) {
        //            if (!Contain(node)) {
        //                new_node_list.push_back(node);
        //            }
        //        }
        // 不知道为什么必须采用下面那种拼法在tree.tig才不会出错
        // 具体问题在Coalesce调用Conservative时出现
        std::list<Node<T> *> new_node_list;
        new_node_list.insert(new_node_list.end(), node_list_.begin(), node_list_.end());
        new_node_list.insert(new_node_list.end(), list.node_list_.begin(), list.node_list_.end());
    ...
    template <typename T>
    NodeList<T> NodeList<T>::Intersected(const NodeList<T> &list) const {
        std::list<Node<T> *> new_node_list;
        for (auto node : list.node_list_) {
            if (Contain(node)) {
                new_node_list.push_back(node);
    ...
    template <typename T>
    NodeList<T> NodeList<T>::Diffed(const NodeList<T> &list) const {
        std::list<Node<T> *> new_node_list;
        for (auto node : node_list_) {
            if (!list.Contain(node)) {
                new_node_list.push_back(node);
            }
        }
        NodeList<T> res;
    ...
    template <typename T>
    NodeList<T> &NodeList<T>::Union(const NodeList<T> &list) {
        node_list_ = Unioned(list).node_list_;
        return *this;
    }

    template <typename T>
    NodeList<T> &NodeList<T>::Intersect(const NodeList<T> &list) {
        node_list_ = Intersected(list).node_list_;
        return *this;
    }

    template <typename T>
    NodeList<T> &NodeList<T>::Diff(const NodeList<T> &list) {
        node_list_ = Diffed(list).node_list_;
        return *this;
    }
    ...
    template <typename T>
    void Graph<T>::RemoveEdge(Node<T> *from, Node<T> *to) {
        assert(from);
        assert(to);
        assert(from->my_graph_ == this);
        assert(to->my_graph_ == this);
        ...
        to->preds_->DeleteNode(from);
        from->succs_->DeleteNode(to);
    }
